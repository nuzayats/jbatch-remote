package org.nailedtothex.jbatchtest2.util;
...
public class MyJobOperatorImpl implements JobOperator {
	...
	public List<StepExecution> getStepExecutions(long jobExecutionId) throws NoSuchJobExecutionException,
		...
		for (StepExecution stepExecution : list1) {
			...
			list2.add(new SimpleStepExecutionImpl(stepExecution.getStepExecutionId(), stepExecution.getStepName(),
					stepExecution.getBatchStatus(), stepExecution.getStartTime(), stepExecution.getEndTime(),
					stepExecution.getExitStatus(), stepExecution.getPersistentUserData(), metrics2));
		}
		return list2;
	}
}
